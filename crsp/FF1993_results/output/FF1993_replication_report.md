...
| --- | --- | --- | --- | --- | --- |
| SMALL | -0.01 | 0.42 | 0.42 | 0.61 | 0.77 |
| ME2 | 0.18 | 0.41 | 0.58 | 0.80 | 0.74 |
| ME3 | 0.17 | 0.76 | 0.63 | 0.73 | 0.88 |
| ME4 | 0.34 | 0.43 | 0.61 | 0.69 | 0.80 |
| BIG | 0.26 | 0.33 | 0.25 | 0.43 | 0.58 |
...
| --- | --- | --- | --- | --- |
| Mkt-RF | 0.32 | 4.77 | 281 | 1.12 |
| SMB | 0.14 | 3.04 | 281 | 0.80 |
| HML | 0.45 | 2.56 | 281 | 2.93 |
| TERM | 1.26 | 1.30 | 342 | 17.88 |
| DEF | 1.11 | 0.48 | 342 | 42.82 |

### Key Findings

- **Size effect**: Within each BE/ME quintile, smaller stocks tend to have higher average excess returns than larger stocks. For example, the smallest growth portfolio (SMALL LoBM) averages -0.01% per month, while the largest growth portfolio (BIG LoBM) averages 0.26%.
- **Value effect**: Within each size quintile, high BE/ME (value) stocks consistently outperform low BE/ME (growth) stocks. The small-value portfolio (SMALL HiBM) averages 0.77% per month versus -0.01% for small-growth.
...
| --- | --- | --- | --- |
| Stocks (25) | 0.7577 | 31.23 | Market factor explains substantial common variation in stock returns |
| Bonds (7) | 0.0256 | 2.54 | Market factor has almost no explanatory power for bond returns |

The market factor alone captures on average 75.8% of the variance in stock portfolio returns, but only 2.56% for bond portfolios. This confirms Fama-French's finding that stocks and bonds are driven by different sources of common risk.
...
| --- | --- | --- |
| Stocks (25) | 0.0570 | Low explanatory power; bond factors do not capture stock variation well |
...
| --- | --- | --- | --- |
| Stocks (25) | 0.9002 | 0.1218 | +14.2 percentage points |
| Bonds (7) | 0.0298 | 0.1535 | Negligible |

The three-factor model raises average R-squared for stocks from 75.8% to 90.0%. SMB and HML loadings are strongly significant across portfolios, confirming the size and value effects documented in the raw returns.
...
| --- | --- | --- | --- |
| Stocks (25) | 0.9010 | +0.08 pp | +14.3 pp |
| Bonds (7) | 0.9047 | +87.49 pp | +87.90 pp |

For stocks, the incremental contribution of TERM and DEF beyond the three stock factors is small (0.08 percentage points), consistent with the original paper. For bonds, adding stock factors to the bond factors provides little additional explanatory power beyond what the bond factors already capture.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Stocks_3Factor | 25 | 3 | 281 | 1.2805 | 0.1733 | 0.1218 | 0.9693 |
| Stocks_5Factor | 25 | 5 | 281 | 1.5501 | 0.0500 | 0.3909 | 1.0259 |
| Bonds_2Factor | 7 | 2 | 281 | 1.5184 | 0.1609 | 0.0064 | 1.6972 |
| Bonds_5Factor | 7 | 5 | 281 | 0.9757 | 0.4492 | 0.0054 | 1.4481 |
| All_5Factor | 32 | 5 | 281 | 1.4385 | 0.0673 | 0.3066 | 1.1183 |

### Key GRS Findings

- **Stocks 3-Factor**: F = 1.2805, p = 0.1733. The three-factor model produces intercepts that are not jointly significant at the 1% level, suggesting it does a reasonable job explaining the cross-section of stock returns.
- **Stocks 5-Factor**: F = 1.5501, p = 0.0500. Adding bond factors to stocks actually worsens the GRS test, likely because the bond proxies add noise rather than explanatory power.
- **Bonds 2-Factor**: F = 1.5184, p = 0.1609. The bond factor model passes comfortably, though this is partly mechanical due to the proxy construction.
- **All 5-Factor**: F = 1.4385, p = 0.0673. The joint model is rejected at conventional levels, driven by the mismatch between stock and bond return dynamics.

### Intercept Analysis Summary

| Model | Mean |alpha| (All) | Mean |alpha| (Stocks) | Mean |alpha| (Bonds) | % Significant |
| --- | --- | --- | --- | --- |
| 1-Factor (Market) | 0.2176 | 0.2355 | 0.1538 | 40.6 |
| 2-Factor (TERM+DEF) | 2.8299 | 3.6210 | 0.0047 | 81.2 |
| 3-Factor (Mkt+SMB+HML) | 0.1288 | 0.1218 | 0.1535 | 31.2 |
| 5-Factor (All) | 0.3066 | 0.3909 | 0.0054 | 15.6 |

The three-factor stock model achieves the lowest mean absolute intercept for stocks (0.1218%), confirming that Mkt-RF, SMB, and HML capture the key dimensions of stock return variation. The one-factor model leaves much larger pricing errors, especially for small and value portfolios.
...
1. **Stock returns are driven by three factors**: The market factor explains much of the time-series variation in stock returns, but adding SMB and HML raises average R-squared from 75.8% to 90.0%. The size and value effects are robust in this sample.
...
4. **Intercept analysis supports the three-factor model for stocks**: The mean absolute intercept for stocks under the three-factor model is 0.1218% per month, substantially lower than the 0.2355% under the one-factor model.
