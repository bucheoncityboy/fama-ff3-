...
1. **채권 요인은 수익률 기반 프록시이다**: TERM과 DEF는 FRED 수익률 스프레드(GS10, TB3MS, BAA, AAA)로 구성되었으며, 실제 채권 포트폴리오 수익률이 아니다. 이는 원 논문 대비 가장 큰 차이점이다.
2. **채권 포트폴리오는 합성적이다**: 7개 채권 포트폴리오는 수익률 스프레드로 구성되었기 때문에, TERM/DEF에 회귀할 때 거의 공선성에 가까워 R² ≈ 1.0이 나타난다. 이는 프록시 구성 방식에서 비롯된 인공적 결과이며, 진정한 복제 결과가 아니다.
